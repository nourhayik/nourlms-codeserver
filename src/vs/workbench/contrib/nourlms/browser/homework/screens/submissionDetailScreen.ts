	...
	implements IHomeworkScreen
{
	...
	private async fetchSubmission(): Promise<void> {
		...
		try {
			...
			// Fast-path: if the upstream eagerly embedded the full AI result
			// inside the submission response (NourLMS does this on
			// `/admin/homeworks/:id/submissions/:sid` — `raw.ai_result`), use
			// it directly instead of doing a separate round-trip to
			// `/ai-result`. This makes the AI grade appear instantly the
			// moment the submission loads.
			const rawForAi = (sub as HomeworkSubmission & { __raw?: unknown }).__raw;
			const embedded = this.extractEmbeddedAiResult(rawForAi);
			if (embedded) {
				this.state.aiResult = embedded;
	...
	private maybeLoadAiResult(): void {
		...
		// If the embedded `ai_result` was already pulled out of the submission
		// response, skip the second round-trip — we already have what we need.
	...
	/**
	 * Pulls a full AI result object out of an embedded submission payload.
	 * The NourLMS upstream attaches this on
	 * `/admin/homeworks/:id/submissions/:sid` as `raw.ai_result` (and is also
	 * sometimes seen as `raw.latest_ai_result` or `raw.grading_result`).
	 *
	 * Returns null when the field is missing, not an object, or doesn't carry
	 * at least an `id` and a `grade` — at which point we fall through to the
	 * separate `/ai-result` fetch.
	 */
	private extractEmbeddedAiResult(raw: unknown): AiGradingResult | null {
		if (!raw || typeof raw !== "object") {
			return null;
		}
		const candidates: unknown[] = [
			(raw as { ai_result?: unknown }).ai_result,
			(raw as { latest_ai_result?: unknown }).latest_ai_result,
			(raw as { grading_result?: unknown }).grading_result,
		];
		for (const c of candidates) {
			if (c && typeof c === "object" && !Array.isArray(c)) {
				const obj = c as { id?: unknown; grade?: unknown };
				if (typeof obj.id === "number" && (typeof obj.grade === "number" || typeof obj.grade === "string")) {
					return c as AiGradingResult;
				}
			}
		}
		return null;
