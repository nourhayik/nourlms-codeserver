	...
	implements IWorkbenchContribution
{
	...
	private _setupTerminalReadOnly(): void {
		...
		const patchInstance = (instance: ITerminalInstance) => {
			...
			const setStdin = (disabled: boolean) => {
				const xterm = instance.xterm;
				if (xterm) {
					xterm.raw.options.disableStdin = disabled;
				}
			};
			...
			// Wire CommandDetection — fires when each command finishes
			...
			// Disable stdin as soon as xterm is ready
			...
			// Re-assert disableStdin=true every 200ms.
			// This catches onPtyReconnect, waitOnExit cleanup, or anything
			// else in VS Code core that sets disableStdin=false.
			...
			// Hook sendText — extensions call this to run code
			...
			// Backup: child process exit detection (works without shell integration)
		...
		// Patch existing terminals (restored from previous session)
		for (const instance of this.terminalService.instances) {
			patchInstance(instance);
		}

		// Patch every new terminal
		this._terminalListener.value = this.terminalService.onDidCreateInstance(
			(instance: ITerminalInstance) => patchInstance(instance),
		);

		// Block new terminal creation while code is running
		...
		// Dispose extra terminals that appear while code runs
