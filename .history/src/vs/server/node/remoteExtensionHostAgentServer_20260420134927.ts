...
class RemoteExtensionHostAgentServer extends Disposable implements IServerAPI {
	...
	public async handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		// GET /nourlms-login-logo.png - login page asset (no auth needed)
	...
	private async _serveLoginLogo(_req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		try {
			const logoPath = FileAccess.asFileUri('vs/server/nourlms-login-logo.png').fsPath;
			...
			this._logService.error('Failed to serve login logo', e);
			return serveError(_req, res, 500, 'Failed to serve login asset.');
		}
	}
