...
export const INourlmsHomeworkPageManager = createDecorator<INourlmsHomeworkPageManager>('nourlmsHomeworkPageManager');
...
export class NourlmsHomeworkPageManager extends Disposable implements INourlmsHomeworkPageManager {
	...
	async open(request: OpenPageRequest): Promise<void> {
		...
		try {
			switch (request.kind) {
				case 'question': {
					const question = request.question ?? await this.api.getQuestion(request.id, cts.token);
				...
				case 'submission': {
					...
					const submission = request.submission ?? await this.fetchSubmission(request.homeworkId, request.id, cts.token);
					htmlBody = await this.renderSubmissionWithResult(submission, request.homeworkId, cts.token);
					break;
				}
				case 'aiResult': {
					const result = request.aiResult ?? await this.api.getAiGradingResult(request.id, cts.token);
					htmlBody = this.renderAiResult(result);
					break;
				}
			}
		} catch (err) {
			this.logService.warn(`[NourlmsHomeworkPageManager] Failed to load page ${key}`, err);
			const errMsg = err instanceof Error ? err.message : String(err);
			htmlBody = `<p style="color:var(--vscode-errorForeground)">${this.escapeHtml(localize('nourlms.homework.page.loadError', "Failed to load content."))}</p>` +
...
<body>${htmlBody}</body>
	...
	private async fetchSubmission(homeworkId: number, submissionId: number, token: CancellationToken): Promise<HomeworkSubmission> {
...
${q.best_answer ? `<div class="field-label">${this.escapeHtml(localize('nourlms.homework.page.bestAnswer', "Best answer"))}</div><div class="code-block">${this.escapeHtml(q.best_answer)}</div>` : ''}`;
	}

	private async renderSubmissionWithResult(s: HomeworkSubmission, homeworkId: number, token: CancellationToken): Promise<string> {
		void homeworkId;
		let aiResultHtml = '';
		if (s.latest_ai_result_id !== null) {
			try {
				const result = await this.api.getAiGradingResult(s.latest_ai_result_id, token);
				aiResultHtml = `
