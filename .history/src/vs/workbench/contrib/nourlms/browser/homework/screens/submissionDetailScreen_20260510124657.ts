...
import { INourlmsHomeworkApi, ApiError, CorrectionPayload } from '../nourlmsHomeworkApi.js';
import { IHomeworkScreen, ScreenContext } from '../nourlmsHomeworkRouter.js';
import type { HomeworkSubmission, AiGradingResult, Homework } from '../types.js';
import { appendErrorRow, appendLoadingRow, formatDate, gradeClassForValue, safeText } from './screenUtils.js';
...
export class SubmissionDetailScreen extends Disposable implements IHomeworkScreen {
	...
	private async fetchSubmission(): Promise<void> {
		...
		try {
			const sub = this.isAdmin
				? await this.api.getAdminSubmission(this.homeworkId, this.submissionId, this.cts.token)
				: await this.api.getStudentSubmission(this.homeworkId, this.submissionId, this.cts.token);
			...
			const merged: HomeworkSubmission = { ...(prev ?? {} as HomeworkSubmission), ...sub };
			if (prev) {
				const keys: (keyof HomeworkSubmission)[] = ['content', 'submitted_at', 'latest_ai_result_id'];
				for (const k of keys) {
					const fresh = (sub as HomeworkSubmission)[k];
					const old = prev[k];
					if ((fresh === null || fresh === undefined || fresh === '') && old !== null && old !== undefined && old !== '') {
			...
			if (!merged.content || merged.content.length === 0) {
				...
				try { rawString = JSON.stringify(raw, null, 2); }
				catch { rawString = String(raw); }
			...
			this.state.submissionLoading = false;
			if (!hadPreload) {
				this.state.submissionError = err instanceof ApiError ? err : new ApiError({ status: 0, message: String(err) });
			}
	...
	private async loadAiResult(): Promise<void> {
		...
		this.refresh();
		try {
			const result = await this.api.getLatestAiResult(this.homeworkId, sub.id, this.cts.token);
			...
			if (err instanceof ApiError && err.status === 404) {
				...
				this.state.aiLoading = false;
				this.state.aiError = err instanceof ApiError ? err : new ApiError({ status: 0, message: String(err) });
			}
			this.refresh();
		}
	}
	...
	private async triggerAiGrade(mode: 'queued' | 'sync'): Promise<void> {
		...
		this.refresh();
		try {
			const result = await this.api.triggerAiGrade(this.homeworkId, { submission_id: sub.id, mode }, this.cts.token);
			...
			this.state.grading = false;
			this.state.gradingError = err instanceof ApiError ? err : new ApiError({ status: 0, message: String(err) });
			this.refresh();
		}
	}
	...
	private async triggerRegrade(): Promise<void> {
		...
		this.refresh();
		try {
			await this.api.triggerRegrade(this.homeworkId, { submission_id: sub.id }, this.cts.token);
			this.state.grading = false;
			setTimeout(() => this.loadAiResult(), 2000);
			this.refresh();
		} catch (err) {
	...
	private async showCorrectModal(): Promise<void> {
		...
		const result = await this.dialogService.input({
			title: localize('nourlms.homework.admin.correct.title', "Correct Submission #{0}", String(sub.id)),
			message: localize('nourlms.homework.admin.correct.message', "Enter the mark and/or feedback for this submission."),
			inputs: [
				{ placeholder: localize('nourlms.homework.admin.correct.mark', "Mark (optional)") },
				{ placeholder: localize('nourlms.homework.admin.correct.feedback', "Feedback (optional)") },
			],
		});
		...
		if (markRaw !== undefined && markRaw !== '') {
			...
			if (!isNaN(mark)) { payload.mark = mark; }
		}
		const feedback = values[1];
		if (feedback !== undefined && feedback !== '') { payload.correct_the_answer = feedback; }

		this.state.correcting = true;
		this.state.correctError = null;
		this.refresh();
		try {
			await this.api.correctSubmission(this.homeworkId, sub.id, payload, this.cts.token);
			this.state.correcting = false;
			this.refresh();
		} catch (err) {
			this.state.correcting = false;
			this.state.correctError = err instanceof ApiError ? err : new ApiError({ status: 0, message: String(err) });
			this.refresh();
		}
	}
	...
	private refresh(): void {
		...
		const sub = this.state.submission;
		if (!sub) {
			appendErrorRow(inner, localize('nourlms.homework.submission.notFound', "Submission not found."));
			return;
		}

		const header = append(inner, $('.nourlms-hw-detail__header'));
		const title = append(header, $('.nourlms-hw-detail__title'));
		title.textContent = localize('nourlms.homework.page.submission', "Submission #{0}", String(sub.id ?? '?'));
		...
		if (this.state.homework?.question) {
			...
			qTitle.textContent = localize('nourlms.homework.submission.question', "Question");
		...
		answerTitle.textContent = localize('nourlms.homework.submission.answer', "Answer");
		...
		if (answerText.trim().length > 0) {
			...
			answerBlock.textContent = localize('nourlms.homework.submission.answerEmpty', "(empty submission)");
			...
			if (this.state.rawDebug !== undefined) {
				let rawString: string;
				try { rawString = JSON.stringify(this.state.rawDebug, null, 2); }
				catch { rawString = String(this.state.rawDebug); }
				const details = append(answerSection, $<HTMLDetailsElement>('details.nourlms-hw-debug'));
				const summary = append(details, $('summary'));
				summary.textContent = localize('nourlms.homework.submission.answerEmpty.debug', "Show raw API response (debug)");
	...
	private renderAiResultSection(parent: HTMLElement): void {
		...
		title.textContent = localize('nourlms.homework.submission.aiResult', "AI Grading Result");

		if (this.state.aiLoading) {
			appendLoadingRow(section, localize('nourlms.homework.submission.aiResultLoading', "Loading AI result…"));
			return;
		}
		...
		if (!r) {
			const note = append(section, $('.nourlms-hw-notice.nourlms-hw-notice--info'));
			note.textContent = localize('nourlms.homework.submission.aiResult.none', "No AI grading result yet.");
			return;
		}
		...
		this.renderResultField(section, localize('nourlms.homework.result.syntaxError', "Syntax error"), r.syntax_error);
		this.renderResultField(section, localize('nourlms.homework.result.hintSyntaxFix', "Syntax fix hint"), r.hint_syntax_fix);
		this.renderResultField(section, localize('nourlms.homework.result.logicalError', "Logical error"), r.logical_error);
		this.renderResultField(section, localize('nourlms.homework.result.hintLogicalFix', "Logical fix hint"), r.hint_logical_fix);
		this.renderResultField(section, localize('nourlms.homework.result.explanation', "Explanation"), r.explanation);
		this.renderResultField(section, localize('nourlms.homework.result.bestAnswerComparison', "Best answer comparison"), r.best_answer_comparison);

		if (r.id !== undefined) {
			const openBtn = append(section, $<HTMLButtonElement>('button.nourlms-hw-button.nourlms-hw-button--ghost.nourlms-hw-button--small'));
			openBtn.type = 'button';
			openBtn.textContent = localize('nourlms.homework.result.openAsPage', "Open as Page");
			openBtn.addEventListener('click', () => {
				this.ctx.push({ kind: 'aiResult', resultId: r.id }, localize('nourlms.homework.page.aiResult', "AI Grading Result #{0}", String(r.id)));
			});
		}
	}

	private renderResultField(parent: HTMLElement, label: string, value: string | null | undefined): void {
		if (!value) { return; }
	...
	private renderAdminActions(parent: HTMLElement): void {
		...
		const queuedBtn = append(actions, $<HTMLButtonElement>('button.nourlms-hw-button'));
		...
		queuedLabel.textContent = localize('nourlms.homework.admin.aiGrade.queued', "Run AI grade (queued)");
		queuedBtn.disabled = this.state.grading;
		queuedBtn.addEventListener('click', () => this.triggerAiGrade('queued'));

		const syncBtn = append(actions, $<HTMLButtonElement>('button.nourlms-hw-button.nourlms-hw-button--ghost'));
		syncBtn.type = 'button';
		syncBtn.textContent = localize('nourlms.homework.admin.aiGrade.sync', "Sync grade");
		syncBtn.disabled = this.state.grading;
		syncBtn.addEventListener('click', () => this.triggerAiGrade('sync'));

		if (this.state.aiResult) {
			const regradeBtn = append(actions, $<HTMLButtonElement>('button.nourlms-hw-button.nourlms-hw-button--ghost'));
			...
			regradeLabel.textContent = localize('nourlms.homework.admin.aiGrade.regrade', "Re-grade");
			regradeBtn.disabled = this.state.grading;
			regradeBtn.addEventListener('click', () => this.triggerRegrade());
		}

		const correctBtn = append(actions, $<HTMLButtonElement>('button.nourlms-hw-button.nourlms-hw-button--secondary'));
		...
		correctLabel.textContent = localize('nourlms.homework.admin.correct', "Mark as corrected");
