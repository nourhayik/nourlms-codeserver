...
class NourlmsStudentRestrictions extends Disposable implements IWorkbenchContribution {
	...
	private _guardActivityBarHidden(): void {
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('workbench.activityBar.location')) {
				const current = this.configurationService.getValue<string>('workbench.activityBar.location');
				if (current !== 'hidden') {
	...
	private _blockContextMenu(): void {
		const handler = (e: MouseEvent) => {
			...
			if (target?.closest('.part.editor') || target?.closest('.part.panel') || target?.closest('.part.sidebar')) {
				return;
			}
			e.preventDefault();
			e.stopPropagation();
	...
	private _blockQuickInput(): void {
		const originalShow = this.quickInputService.quickAccess.show.bind(this.quickInputService.quickAccess);
		(this.quickInputService.quickAccess as any).show = (prefix: string, options?: any) => {
	...
	private _blockThemeChanges(): void {
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration('workbench.colorTheme')) {
				const current = this.configurationService.getValue<string>('workbench.colorTheme');
				if (current !== 'Default Dark Modern') {
					this.configurationService.updateValue('workbench.colorTheme', 'Default Dark Modern');
				}
			}
		}));
	}
	...
	private _setupTerminalReadOnly(): void {
		...
		const patchInstance = (instance: ITerminalInstance) => {
			...
			let stdinLocked = true;

			// Intercept xterm.raw.options.disableStdin so VS Code core code
			// (onPtyReconnect, waitOnExit cleanup, etc.) cannot flip it to false.
			// We replace the property with a getter/setter we control.
			const installOptionsGuard = (rawOptions: any) => {
				let realValue = true;
				Object.defineProperty(rawOptions, 'disableStdin', {
					get: () => realValue,
					set: (v: boolean) => {
						if (!v && stdinLocked) {
							return;
			...
			const lockTerminal = () => {
				...
				if (xterm) {
					(xterm.raw.options as any).disableStdin = true;
				}
			};
			...
			const markRunStart = () => {
				if (!commandRunning) {
					...
					sawChildren = false;
					unlockTerminal();
					this.logService.info('[NourLMS] Terminal: code execution started, stdin enabled');
				}
			};
			...
			const markRunEnd = () => {
				if (commandRunning) {
					...
					sawChildren = false;
					lockTerminal();
					this.logService.info('[NourLMS] Terminal: code execution finished, stdin disabled');
				}
			};

			// Wire CommandDetection — fires per finished command (e.g. `python script.py`)
			const wireCommandDetection = () => {
				const cmdDetect = instance.capabilities.get(TerminalCapability.CommandDetection);
				if (cmdDetect) {
					store.add(cmdDetect.onCommandFinished(() => markRunEnd()));
				}
				const partialDetect = instance.capabilities.get(TerminalCapability.PartialCommandDetection);
				if (partialDetect) {
					store.add(partialDetect.onCommandFinished(() => markRunEnd()));
				}
			};
			store.add(instance.capabilities.onDidAddCommandDetectionCapability(() => wireCommandDetection()));
			wireCommandDetection();

			// Once xterm is ready: install the options guard and lock
			instance.xtermReadyPromise.then(xterm => {
				if (!xterm) { return; }
				installOptionsGuard(xterm.raw.options);
				if (!commandRunning) {
			...
			(instance as any).sendText = function (text: string, shouldExecute?: boolean, bracketedPasteMode?: boolean) {
				if (shouldExecute !== false) {
					markRunStart();
				}
				return originalSendText(text, shouldExecute ?? true, bracketedPasteMode);
			};

			const originalSendPath = (instance as any).sendPath?.bind(instance);
			if (originalSendPath) {
				(instance as any).sendPath = function (originalPath: string | any, shouldExecute?: boolean) {
					markRunStart();
			...
			store.add(instance.onDidChangeHasChildProcesses((hasChildren: boolean) => {
				if (hasChildren) {
					sawChildren = true;
		...
		const originalCreateTerminal = this.terminalService.createTerminal.bind(this.terminalService);
		(this.terminalService as any).createTerminal = (config?: any) => {
			if (codeIsRunning) {
				this.logService.info('[NourLMS] Blocked terminal creation — code is running');
	...
	private _closeWelcomePage(): void {
		for (const group of this.editorGroupsService.groups) {
			const welcomeEditors = group.editors.filter(e => e.typeId === gettingStartedInputTypeId);
