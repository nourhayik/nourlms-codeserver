...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	/**
	 * NourLMS: when true, keyboard input is blocked and onPtyReconnect
	 * will not re-enable stdin.  Only programmatic sendText() with
	 * {@link nourlmsAllowInput} temporarily bypasses the block.
	 */
	nourlmsReadOnly: boolean = false;
	nourlmsAllowInput: boolean = false;
	...
	get reconnectionProperties(): IReconnectionProperties | undefined {
		return (
			this.shellLaunchConfig.attachPersistentProcess?.reconnectionProperties ||
	...
	constructor(
		...
		if (this.shellLaunchConfig.cwd) {
			const cwdUri = isString(this._shellLaunchConfig.cwd)
				? URI.from({
						scheme: Schemas.file,
		...
		this._terminalHasTextContextKey = TerminalContextKeys.textSelected.bindTo(
		...
		this._register(
			this.capabilities.onDidAddCapability((e) =>
				this._logService.debug("terminalInstance added capability", e.id),
			),
		);
		this._register(
			this.capabilities.onDidRemoveCapability((e) =>
				this._logService.debug("terminalInstance removed capability", e.id),
			),
		);

		const capabilityListeners = this._register(
			new DisposableMap<TerminalCapability, IDisposable>(),
		);
		this._register(
			this.capabilities.onDidAddCapability((e) => {
				...
				switch (e.id) {
					...
					case TerminalCapability.CommandDetection: {
						...
						store.add(
							e.capability.onCommandExecuted(async (command) => {
								...
								if (!command.id && command.command) {
									...
									this.xterm?.shellIntegration.setNextCommandId(
										command.command,
										commandId,
									);
									await this._processManager.setNextCommandId(
										command.command,
										commandId,
									);
								}
					...
					case TerminalCapability.PromptTypeDetection: {
						capabilityListeners.set(
							e.id,
							e.capability.onPromptTypeChanged(refreshInfo),
						);
						break;
					}
				}
		...
		this._xtermReadyPromise
			.then(async () => {
				...
				if (os && this.shellLaunchConfig.executable) {
					this.setShellType(
						guessShellTypeFromExecutable(os, this.shellLaunchConfig.executable),
					);
				}
	...
	private async _handleOnData(data: string): Promise<void> {
		if (this.nourlmsReadOnly && !this.nourlmsAllowInput) {
			return;
		}
	...
	protected async _createXterm(): Promise<XtermTerminal | undefined> {
		...
		const xterm = this._scopedInstantiationService.createInstance(
			...
			Terminal,
			{
				cols: this._cols,
				rows: this._rows,
				xtermColorProvider: this._scopedInstantiationService.createInstance(
		...
		this._register(
			xterm.raw.onSelectionChange(() => this._onDidChangeSelection.fire(this)),
		);
		this._register(
		...
		this._register(
			this._processManager.onProcessReady(async (processTraits) => {
				...
				if (processTraits?.windowsPty?.backend === "conpty") {
					this._register(
						xterm.raw.parser.registerCsiHandler({ final: "c" }, (params) => {
							if (
								params.length === 0 ||
								(params.length === 1 && params[0] === 0)
							) {
		...
		this._register(
			xterm.onDidChangeProgress(() => this._labelComputer?.refreshLabel(this)),
		);

		// Register and update the terminal's shell integration status
		this._register(
			Event.runAndSubscribe(
				xterm.shellIntegration.onDidChangeSeenSequences,
				() => {
		...
		if (!this.capabilities.has(TerminalCapability.CwdDetection)) {
			...
			this._register(
				this.capabilities.onDidAddCwdDetectionCapability(() => {
	...
	private _refreshShellIntegrationInfoStatus(instance: ITerminalInstance) {
		...
		const seenSequences = Array.from(
			instance.xterm.shellIntegration.seenSequences,
		);
		if (seenSequences.length > 0) {
			detailedAdditions.push(
				`Seen sequences: ${seenSequences.map((e) => `\`${e}\``).join(", ")}`,
			);
		}
	...
	async runCommand(
		...
		if (
			!commandDetection ||
			commandDetection.promptInputModel.state !== PromptInputState.Input
		) {
			const store = new DisposableStore();
			...
			await Promise.race([
				new Promise<void>((r) => {
					store.add(
						this.capabilities.onDidAddCommandDetectionCapability((e) => {
							commandDetection = e;
							if (
								commandDetection.promptInputModel.state ===
								PromptInputState.Input
							) {
								r();
							} else {
								store.add(
		...
		if (
			shouldExecute &&
			(!commandDetection || commandDetection.promptInputModel.value.length > 0)
		) {
	...
	protected _createProcessManager(): TerminalProcessManager {
		...
		const processManager = this._scopedInstantiationService.createInstance(
			...
			this.shellLaunchConfig.shellIntegrationNonce ??
				...
				if (!this._labelComputer) {
					...
					this._register(
						this._labelComputer.onDidChangeLabel((e) => {
							const wasChanged =
								this._title !== e.title || this._description !== e.description;
							if (wasChanged) {
								this._title = e.title;
			...
			processManager.onDidChangeProperty(({ type, value }) => {
				switch (type) {
					...
					case ProcessPropertyType.Title:
						this._setTitle(
							(value as IProcessPropertyMap[ProcessPropertyType.Title]) ?? "",
							TitleEventSource.Process,
						);
						break;
					case ProcessPropertyType.OverrideDimensions:
						this.setOverrideDimensions(
							value as IProcessPropertyMap[ProcessPropertyType.OverrideDimensions],
							true,
						);
						break;
					case ProcessPropertyType.ResolvedShellLaunchConfig:
						this._setResolvedShellLaunchConfig(
							value as IProcessPropertyMap[ProcessPropertyType.ResolvedShellLaunchConfig],
						);
						break;
					case ProcessPropertyType.ShellType:
						this.setShellType(
							value as IProcessPropertyMap[ProcessPropertyType.ShellType],
						);
						break;
					case ProcessPropertyType.HasChildProcesses:
						this._onDidChangeHasChildProcesses.fire(
							value as IProcessPropertyMap[ProcessPropertyType.HasChildProcesses],
						);
						break;
					...
					case ProcessPropertyType.ShellIntegrationInjectionFailureReason:
						this._shellIntegrationInjectionInfo =
							value as IProcessPropertyMap[ProcessPropertyType.ShellIntegrationInjectionFailureReason];
						break;
				}
			}),
		);

		this._initialDataEventsListener.value = processManager.onProcessData((ev) =>
		...
		this._register(
			processManager.onPtyReconnect(() => {
				if (this.xterm && !this.nourlmsReadOnly) {
	...
	private async _createProcess(): Promise<void> {
		...
		if (
			...
			this._cwd &&
			this._userHome &&
			normalizeDriveLetter(this._cwd) !== normalizeDriveLetter(this._userHome)
		) {
	...
	private _onProcessData(ev: IProcessDataEvent): void {
		...
		const matches = ev.data.matchAll(
			/(?<seq>\x1b\][16]33;(?:C|D(?:;\d+)?)\x07)/g,
		);
		let i = 0;
		...
		if (ev.trackCommit) {
			ev.writePromise = new Promise<void>((r) =>
				this._writeProcessData(lastData, r),
			);
		} else {
	...
	private async _onProcessExit(
		...
		if (
			...
			this._processManager.processState === ProcessState.KilledDuringLaunch &&
				...
				switch (typeof waitOnExit) {
					...
					case "function":
						if (this.exitCode !== undefined) {
							xterm.raw.write(
								formatMessageForTerminal(waitOnExit(this.exitCode), {
									excludeLeadingNewLine: true,
								}),
							);
						}
	...
	private _flushXtermData(): Promise<void> {
		...
		return new Promise<void>((r) => {
			const interval = dom.disposableWindowInterval(
				...
				() => {
					if (
						this._latestXtermWriteData === this._latestXtermParseData ||
						++retries === 5
					) {
						interval.dispose();
						r();
	...
	private async _updatePtyDimensions(rawXterm: XTermTerminal): Promise<void> {
		...
		const roundedPixelHeight = pixelHeight
	...
	private _setAriaLabel(
		...
		if (xterm && xterm.textarea) {
			if (title && title.length > 0) {
				...
				labelParts.push(
					nls.localize("terminalTextBoxAriaLabel", "Terminal {0}", terminalId),
				);
			}
	...
	private async _refreshEnvironmentVariableInfoWidgetState(
		...
		if (
			...
			!(
				...
				this._processManager.processTraits?.windowsPty?.buildNumber &&
				this._processManager.processTraits.windowsPty.buildNumber >= 18309
			)
		) {
			this.relaunch();
			return;
	...
	private _setTitle(
		...
		if (
			(this._shellLaunchConfig?.type === "Task" ||
...
export class TerminalLabelComputer extends Disposable {
	...
	computeLabel(
		...
		const templateProperties: ITerminalLabelTemplateProperties = {
			...
			shellPromptInput:
				commandDetection?.executingCommand && promptInputModel
...
export function parseExitResult(
	...
	const code = isNumber(exitCodeOrError)
		? exitCodeOrError
	...
	switch (typeof exitCodeOrError) {
		...
		case "object": {
			...
			if (conptyError) {
				const errorCode =
					conptyError.length > 1 ? parseInt(conptyError[1]) : undefined;
				switch (errorCode) {
					case 5:
...
function guessShellTypeFromExecutable(
	os: OperatingSystem,
	executable: string,
